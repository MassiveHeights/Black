...
class DistortionEffect extends SoundEffect {
  constructor(value = 0.5) {
    super();
    this.mWaveShaperNode = Audio.context.createWaveShaper();
    this.mInputNode = this.mOutputNode = this.mWaveShaperNode;

    this.samples = 44100;
    this.curve = new Float32Array(this.samples);
  ...
  set distortion(value) {
    ...
    this.makeDistortionCurve(this.mValue, this.samples, this.curve);
    this.mWaveShaperNode.curve = this.curve;
  }
  ...
  makeDistortionCurve(amount, n_samples, curve) {
    let k = ~~(amount * 100);
    let deg = Math.PI / 180;
    let x = 0;
