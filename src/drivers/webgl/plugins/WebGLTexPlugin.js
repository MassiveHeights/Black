...
class WebGLTexPlugin extends WebGLBasePlugin {
  constructor(renderer) {
    ...
    gl.vertexAttribPointer(location.aTexSlot, 1, gl.FLOAT, false, 20, 16);  // todo check
  ...
  flush() {
    ...
    for (i = 0; i < length; i++) {
      ...
      (object.mDirty & DirtyFlag.WORLD) && object.refreshVertexData();
      ...
      if (currentBatchSlots[texture.mId] === undefined) {
    ...
    for (let i = 0, len = currentBatchIndex + 1; i < len; i++) {
      ...
      for (let j = 0, l = batch.texturesLength; j < l; j++) {
        ...
        const slot = slots[texture.mId];
        slots[texture.mId] = undefined;
